...
from framework_kg import Model
import models_kg
import config
...
def collate_fn(batch):
    data, label,aspects,_ = zip(*batch)
    user_reviews= []
    item_reviews= []
    user_ids= []
    item_ids= []
    ...
    item_docs = []
    for d in data:
        user_review, item_review, user_id, item_id,ent_user_id,ent_item_id, user_item2id, item_user2id, user_doc, item_doc,_,_,_,_ = d
    ...
    return [user_reviews, item_reviews, user_ids, item_ids,ent_user_ids,ent_item_ids, user_item2ids, item_user2ids, user_docs, item_docs], label
...
def train(**kwargs):
    ...
    for epoch in range(opt.num_epochs):
            ...
            if opt.fine_step:
                if idx % opt.print_step == 0 and idx > 0:
                    ...
                    if val_mse < best_res:
                        best_res = val_mse
                        patience = 0
                        name = model.save(name=opt.dataset, opt=opt.print_opt + opt.version)
        ...
        val_loss, val_mse, val_mae = predict(model, val_data_loader, opt,'val')
        test_loss, test_mse, test_mae = predict(model, test_data_loader, opt,'test')
    ...
    model.load(name)
    val_loss, val_mse, val_mae = predict(model, test_data_loader, opt,'test')
...
def predict(model, data_loader, opt,subset):
    total_loss = 0.0
    total_maeloss = 0.0
    model.eval()
    with torch.no_grad():
