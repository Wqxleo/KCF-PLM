...
from framework_aspect import Model,SigmoidFocalClassificationLoss
...
def train(**kwargs):
    ...
    #     {'params': model.rating_encoder.parameters(), 'lr': opt.lr},
    #     {'params': model.aspect_encoder.parameters(), 'lr': opt.lr},
    #     {'params': model.RGCN.parameters(), 'lr': opt.ent_lr},
    #     {'params': model.predict_net.parameters(), 'lr': opt.lr},
    #     {'params': model.ent_fc_u.parameters(), 'lr': opt.lr},
    #     {'params': model.ent_fc_i.parameters(), 'lr': opt.lr},
    #     {'params': model.ent_fc_a.parameters(), 'lr': opt.lr},
    #
    ...
    for epoch in range(opt.num_epochs):
        ...
        for idx, (train_datas, scores, aspects,aspect_label_one_hots) in enumerate(train_data_loader):
            ...
            train_datas = tuple([torch.LongTensor(t).cuda() for t in train_datas[:-1]]+[torch.FloatTensor(train_datas[-1]).cuda()])
            optimizer.zero_grad()
            if len(opt.gpu_ids) > 0:
                output,aspect_pre = modelP(train_datas)
            else:
                output, aspect_pre = model(train_datas)
            ...
            if opt.fine_step:
                if idx % opt.print_step == 0 and idx > 0:
                    ...
                    if val_mse < best_res:
                        best_res = val_mse
                        patience = 0
                        name = model.save(name=opt.dataset, opt='aspect' + opt.version)
        ...
        subset = 'val'
        val_loss, val_mse, val_mae = predict(model, val_data_loader, opt,aspects_count,subset)
        test_loss, test_mse, test_mae = predict(model, test_data_loader, opt, aspects_count, 'test')
